...
//   PhoneOff,
// } from "lucide-react";
// // If you have AuthContext, you can import it; otherwise comment out:
...
//   // Optional schedule info from URL
//   const scheduledDate = searchParams.get("date") || "";
//   const timeSlot = searchParams.get("timeSlot") || "";
//   const role = searchParams.get("role") || "";
//   const jobId = searchParams.get("jobId") || "";
//   const defaultRoomFromUrl =
//     searchParams.get("room") || (jobId ? `Interview_${jobId}` : "InterviewMateRoom");

//   useEffect(() => {
//     setMounted(true);
//     setRoomName(defaultRoomFromUrl);
//   }, [defaultRoomFromUrl]);
...
//               <span className="font-semibold text-slate-900">30 minutes</span>. A timer
//               will run after you join.
//             </p>
//           </div>
//         </div>
//       </section>
...
//                       other person should join the same room ID.
//                     </p>
//                   </>
//                 )}
//                 {callEnded && (
...
export const metadata = {
  title: "Interview Call Room | InterviewMate",
  description:
    "Live video calling room for HR and candidates to join scheduled interview sessions.",
};

// This tells Next not to try to fully pre-render it as static
export const dynamic = "force-dynamic";

export default function CallingPage({ searchParams }) {
  const scheduledDate = searchParams?.date ?? "";
  const timeSlot = searchParams?.timeSlot ?? "";
  const role = searchParams?.role ?? "";
  const jobId = searchParams?.jobId ?? "";
  const roomFromUrl = searchParams?.room ?? "";

  return (
    <Suspense
      fallback={
        <main className="min-h-screen flex items-center justify-center bg-slate-50">
          <p className="text-sm text-slate-600">Loading call room...</p>
        </main>
      }
    >
      <CallingClient
